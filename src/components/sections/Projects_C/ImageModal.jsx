...
export function ImageModal({ images, initialIndex = 0, title = 'Galeria de proyecto', onClose }) {
	const MIN_ZOOM = 1;
	const MAX_ZOOM = 4;
	...
	}, [applyZoom, handleNext, handlePrev, onClose, resetZoom, zoom]);
	...
	return createPortal(
		...
		>
			...
			>
				<h3 className="absolute left-3 top-3 z-20 max-w-[70%] rounded-md bg-black/60 px-3 py-2 text-sm font-semibold text-white sm:left-6 sm:top-6 sm:text-base">
					{title}
				</h3>
				...
				<button
					...
					aria-label="Cerrar galeria de proyectos"
				>
							...
							>
								...
								>
									<img
										...
										alt={`Imagen de proyecto ${currentIndex + 1} de ${images.length}`}
										className={`mx-auto h-auto max-h-[70dvh] w-full select-none object-contain sm:max-h-[76dvh] ${zoom > MIN_ZOOM ? (isPanning ? 'cursor-grabbing' : 'cursor-grab') : 'cursor-zoom-in'}`}
										loading="lazy"
										draggable={false}
										style={{
						...
						<div className="hidden max-w-full flex-wrap justify-center gap-3 px-2 sm:flex">
							{images.map((img, index) => (
								<button
									...
									aria-label={`Ver imagen ${index + 1} de proyecto`}
